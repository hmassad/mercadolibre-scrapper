...
import com.mercadolibre.ApiClient;
import com.mercadolibre.ApiException;
import com.mercadolibre.Configuration;
import com.mercadolibre.Pair;
...
public class DefaultApi {
    ...
    public List<ItemResponse> search(String accessToken, String site, String category, String query) throws ApiException {

        final int limit = 50;
        int offset = 0;

        String localVarPath = "/sites/" + site + "/search";
        ...
        if (query != null)
            localVarQueryParams.addAll(apiClient.parameterToPairs("", "q", query));
        Pair offsetPair = apiClient.parameterToPairs("", "offset", offset).get(0);
        localVarQueryParams.add(offsetPair);

        List<ItemResponse> items = new ArrayList<>();
        do {
            SearchResponse searchResponse = apiClient.invokeAPI(localVarPath, "GET", localVarQueryParams, null, localVarHeaderParams, localVarFormParams, localVarAccept, localVarContentType, localVarAuthNames, new GenericType<SearchResponse>() {
            });
            if (searchResponse != null)
                items.addAll(searchResponse.getResults());

            // actualizar offset: sacar offset pair viejo y poner uno nuevo
            offset += limit;
            localVarQueryParams.remove(offsetPair);
            offsetPair = apiClient.parameterToPairs("", "offset", offset).get(0);
            localVarQueryParams.add(offsetPair);
